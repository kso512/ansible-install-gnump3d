...
# Check for the GNUMP3d process
docker exec --tty $container_id env TERM=xterm ps -ef | grep gnump3 | grep -v grep

# Check for the GNUMP3d HTTPD out port
docker exec $container_id ss -lnt | grep 8888
